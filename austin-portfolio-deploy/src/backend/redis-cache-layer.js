...
class RedisCacheLayer {
  ...
  async initialize() {
    // Skip Redis initialization if not configured or explicitly disabled
    if (process.env.DISABLE_REDIS === 'true' || (!process.env.REDIS_URL && !process.env.REDIS_HOST)) {
      console.log('📦 Using in-memory cache only (Redis disabled/unavailable)');
      this.isConnected = false;
      return;
    }
    ...
    try {
      ...
      this.redis.on('error', (error) => {
        ...
        // Disable Redis after 3 failed attempts
