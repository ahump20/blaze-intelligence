...
class NILValuationEngine {
    ...
    calculateValuation() {
        ...
        // Apply SEC market intelligence multipliers
        const secMultiplier = level === 'd1' ? 1.43 : 1.0; // SEC market premium
        const texasMultiplier = sport === 'football' ? 1.67 : 1.0; // Texas advantage
        const finalValue = totalValue * secMultiplier * texasMultiplier;
        ...
        const minValue = Math.max(finalValue * (1 - variance * 0.3), 1000); // Minimum $1,000
        const maxValue = finalValue * (1 + variance * 0.3);
