...
class MomentumEngine {
  ...
  // Extract features for ML models (College Football)
  extractFeatures(gameState) {
    const features = {
      // Temporal features
      timeRemaining: gameState.timeRemaining,
      quarter: gameState.quarter,
      playClock: gameState.playClock || 40,
      ...
      // Possession features (CFB-specific)
      possessionTime: gameState.possessionTime,
      down: gameState.down || 1,
      distance: gameState.distance || 10,
      yardline: gameState.yardline || 50,
      fieldPosition: this.calculateFieldPosition(gameState.yardline),
      turnovers: gameState.turnovers,
      turnoverDifferential: gameState.turnoverDifferential || 0,
      
      // CFB Drive features
      driveLength: gameState.driveLength || 0,
      driveTime: gameState.driveTime || 0,
      timeOfPossession: gameState.timeOfPossession || 0,
      firstDowns: gameState.firstDowns || 0,
      thirdDownConversions: gameState.thirdDownConversions || 0,
      thirdDownAttempts: gameState.thirdDownAttempts || 0,
      
      // CFB Red Zone and Fourth Down
      redZoneAttempts: gameState.redZoneAttempts || 0,
      redZoneConversions: gameState.redZoneConversions || 0,
      fourthDownAttempts: gameState.fourthDownAttempts || 0,
      fourthDownConversions: gameState.fourthDownConversions || 0,
      
      // Player/Team features
      starPlayerOn: gameState.starPlayerActive ? 1 : 0,
      specialTeamsPoints: gameState.specialTeamsPoints || 0,
      timeoutsRemaining: gameState.timeoutsRemaining || 3,
      penaltyYards: gameState.penaltyYards || 0,
      ...
      redZoneEfficiency: this.calculateRedZoneEfficiency(gameState),
      thirdDownEfficiency: this.calculateThirdDownEfficiency(gameState)
    };
  ...
  predictWithXGBoost(features) {
    ...
    const weights = {
      leverageIndex: 0.20,
      scoreDifferential: 0.18,
      fieldPosition: 0.15,
      timeRemaining: 0.12,
      redZoneEfficiency: 0.10,
      thirdDownEfficiency: 0.08,
      turnoverDifferential: 0.07,
      consecutiveScores: 0.05,
      fourthDownConversions: 0.03,
      starPlayerOn: 0.02
    };
    
    let prediction = 0.5;  // Start neutral
    
    // Apply weighted features (CFB-focused)
    prediction += (features.leverageIndex - 0.5) * weights.leverageIndex;
    prediction += (Math.tanh(features.scoreDifferential / 14)) * weights.scoreDifferential; // 14-point CFB scoring range
    prediction += features.fieldPosition * weights.fieldPosition;
    prediction += ((features.timeRemaining / 3600) - 0.5) * weights.timeRemaining; // 60-minute CFB game
    prediction += features.redZoneEfficiency * weights.redZoneEfficiency;
    prediction += features.thirdDownEfficiency * weights.thirdDownEfficiency;
    prediction += (Math.tanh(features.turnoverDifferential / 2)) * weights.turnoverDifferential;
    prediction += (Math.tanh(features.consecutiveScores / 2)) * weights.consecutiveScores;
    prediction += (features.fourthDownConversions > 0 ? 0.1 : -0.05) * weights.fourthDownConversions;
  ...
  // Predict next likely play type (CFB-specific)
  ...
  // Calculate field position advantage (-1 to 1, where 1 is opponent's goal line)
  calculateFieldPosition(yardline) {
    if (!yardline) return 0;
    // Convert yardline to field position value
    // yardline 1-20: excellent field position (0.6-1.0)
    // yardline 21-50: good field position (0.2-0.6)  
    // yardline 51-80: neutral field position (-0.2-0.2)
    // yardline 81-99: poor field position (-0.6--0.2)
    return Math.max(-1, Math.min(1, (100 - yardline) / 50 - 1));
  }

  // Calculate red zone efficiency (completion rate)
  calculateRedZoneEfficiency(gameState) {
    const attempts = gameState.redZoneAttempts || 0;
    const conversions = gameState.redZoneConversions || 0;
    if (attempts === 0) return 0.5; // neutral
    return Math.min(1, conversions / attempts);
  }

  // Calculate third down efficiency
  calculateThirdDownEfficiency(gameState) {
    const attempts = gameState.thirdDownAttempts || 0;
    const conversions = gameState.thirdDownConversions || 0;
    if (attempts === 0) return 0.5; // neutral
    return Math.min(1, conversions / attempts);
  }

  initializeFeatureExtractors() {
    // Set up feature extraction pipelines (CFB-focused)
    this.featureExtractors = {
      temporal: (state) => ({
        time: state.timeRemaining,
        quarter: state.quarter,
        playClock: state.playClock
      }),
      ...
      contextual: (state) => ({
        ...
        conferenceGame: state.isConferenceGame
      })
    };
  }
